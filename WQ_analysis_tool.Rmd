...
The table below shows summary results for all sites across all data sources (lab, field and probe data). The first page shows the results for all data except the exclusions noted above, while page 2 shows the results with the flagged outliers also excluded.
...
full_data <- full_data %>%
  dplyr::mutate(status = case_when(HIGH > POOR ~ ifelse(result > HIGH, "High",
                                   ...
                                   POOR > HIGH ~ ifelse(result > POOR, "BAD",
                                   ...
                                   parameter == "pH" ~ ifelse(result > 6 & result < 9, "High", "Not high"),
