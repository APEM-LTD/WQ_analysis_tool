...
full_data <- full_data %>%
  dplyr::mutate(std_order = case_when(HIGH > POOR ~ "asc",
                                      POOR > HIGH ~ "desc",
                                      parameter == "pH" ~ "pH",
                                      .default = "none"),
                status = case_when(std_order == "asc" ~   ifelse(result > HIGH, "High",
                                   ...
                                   std_order == "desc" ~  ifelse(result > POOR, "BAD",
  ...
  for (p in unique(full_data$parameter)) {
    ...
    if (unique(chart_data$std_order == "desc")){
      chart<-ggplot(data = chart_data) +
        geom_rect(aes(ymin = y_min_val, ymax = HIGH, xmin = -Inf, xmax = Inf), fill = '#B7DEE8', alpha = 1) +
        geom_rect(aes(ymin = HIGH, ymax = GOOD, xmin = -Inf, xmax = Inf), fill = '#D8E4BC', alpha = 1) +
        geom_rect(aes(ymin = GOOD, ymax = MODERATE, xmin = -Inf, xmax = Inf), fill = '#FFFF66', alpha = 1) +
        geom_rect(aes(ymin = MODERATE, ymax = POOR, xmin = -Inf, xmax = Inf), fill = '#FFCC66', alpha = 1) +
        geom_rect(aes(ymin = POOR, ymax = y_max_val, xmin = -Inf, xmax = Inf), fill = '#FF9999', alpha = 1) +
    ...
    if(unique(chart_data$std_order == "asc")){
      chart<-ggplot(data = chart_data) +
        geom_rect(aes(ymin = y_min_val, ymax = POOR, xmin = -Inf, xmax = Inf), fill = '#FF9999', alpha = 1) +
        geom_rect(aes(ymin = POOR, ymax = MODERATE, xmin = -Inf, xmax = Inf), fill = '#FFCC66', alpha = 1) +
        geom_rect(aes(ymin = MODERATE, ymax = GOOD, xmin = -Inf, xmax = Inf), fill = '#FFFF66', alpha = 1) +
        geom_rect(aes(ymin = GOOD, ymax = HIGH, xmin = -Inf, xmax = Inf), fill = '#D8E4BC', alpha = 1) +
        geom_rect(aes(ymin = HIGH, ymax = y_max_val, xmin = -Inf, xmax = Inf), fill = '#B7DEE8', alpha = 1) +
        geom_point(aes(x = lubridate::year(date), y = result, shape = season)) +
        scale_y_continuous(limits = c(y_min_val, y_max_val), breaks = pretty_breaks()) +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
        scale_shape_manual(values = c(3,4,1, 5)) +
    ...
    if(unique(chart_data$std_order == "pH")){
      chart<-ggplot(data = chart_data) +
        geom_rect(aes(ymin = 6, ymax = 9, xmin = -Inf, xmax = Inf), fill = '#B7DEE8', alpha = 1) +
        geom_point(aes(x = lubridate::year(date), y = result, shape = season)) +
