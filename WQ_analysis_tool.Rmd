...
pacman::p_load(remotes, readxl, dplyr, stringr, lubridate, ggplot2, writexl, 
               ...
               rlang, readr, openxlsx, scales, htmltools, dygraphs) # new for WQ tool
...
colnames(summary_tab_data) <- c("Location", "Parameter", "Mean", "SD", "Min", "Max", "Percentile", "Percentile value", "Source")

summary_tab <- DT::datatable(summary_tab_data, extensions = "Buttons", 
                                     options = list(
                                       pageLength = length(PARAMETERS) * length(unique(summary_tab_data$Location)),
                                       dom = 'Bfrtip',
  ...
  for (p in unique(full_data$parameter)) {
    ...
    chart_data <- full_data %>%
      ...
      dplyr::mutate(x_min_val = as.Date(paste0(as.character(lubridate::year(min(date))), "-01-01")),
                    ...
                    y_min_val = min(result, HIGH - 10, POOR, -10, na.rm = TRUE),
                    y_max_val = max(result, HIGH +10, POOR +10, na.rm = TRUE))
    
    y_min_val <- min(chart_data$result, chart_data$HIGH - 10, chart_data$POOR, -10, na.rm = TRUE)
    y_max_val <- max(chart_data$result, chart_data$HIGH +10, chart_data$POOR +10, na.rm = TRUE)
    x_min_val <- unique(chart_data$x_min_val)
    x_max_val <- unique(chart_data$x_max_val)
    ...
    if (unique(chart_data$std_order == "asc")){
      chart<-ggplot(data = chart_data) +
        geom_rect(data = subset(chart_data, main_trib == "tributary")[1,], aes(fill = "#D9D9D9", xmin = x_min_val, xmax = x_max_val, ymin = y_min_val, ymax = y_max_val),  alpha = 0.5) +
        ...
        geom_hline(aes(yintercept = HIGH), colour = '#B7DEE8', linetype = 2, linewidth = 1.2) +
        geom_hline(aes(yintercept = GOOD), colour = '#D8E4BC', linetype = 2, linewidth = 1.2) +
        geom_hline(aes(yintercept = MODERATE), colour = '#FFFF66', linetype = 2, linewidth = 1.2) +
        geom_hline(aes(yintercept = POOR), colour = '#FFCC66', linetype = 2, linewidth = 1.2) +
        geom_point(aes(x = as.Date(date), y = result, shape = source)) +
        scale_y_continuous(limits = c(y_min_val, y_max_val), breaks = pretty_breaks()) +
        scale_fill_manual(values="#D9D9D9", labels=c("Tributary river"), name = "") +
        theme_bw() +
        ...
        facet_grid(parameter~location_name, switch = "y", scales = "free_y")
    ...
    if(unique(chart_data$std_order == "desc")){
      chart<-ggplot(data = chart_data) +
        geom_rect(data = subset(chart_data, main_trib == "tributary")[1,], aes(fill = main_trib, xmin = x_min_val, xmax = x_max_val, ymin = -Inf, ymax = Inf), fill = "grey", alpha = 0.5) +
    ...
    if(unique(chart_data$std_order == "pH")){
      chart<-ggplot(data = chart_data) +
        geom_rect(data = subset(chart_data, main_trib == "tributary")[1,], aes(fill = main_trib, xmin = x_min_val, xmax = x_max_val, ymin = 0, ymax = 15), fill = "grey", alpha = 0.5) +
    ...
    if(unique(chart_data$std_order == "none")){
      chart<-ggplot(data = chart_data) +
        geom_rect(data = subset(chart_data, main_trib == "tributary")[1,], aes(fill = main_trib, xmin = x_min_val, xmax = x_max_val, ymin = y_min_val, ymax = y_max_val), fill = "grey", alpha = 0.5) +
        geom_point(aes(x = as.Date(date), y = result, shape = source)) +
        scale_y_continuous(limits = c(y_min_val, y_max_val), breaks = pretty_breaks()) +
        theme(axis.text.x = element_text(angle = 45, vjust = 0.5)) +
        scale_shape_manual(values = c(3,4,1,5)) +
    ...
    c <- plotly::ggplotly(chart) %>%
      layout(autosize = FALSE, width = 800, margin = list(l = 50, r = 50, b = 110, t = 110, pad = 4))
    charts[[counter]] <- c
    counter <- counter + 1
...
wb <- openxlsx::buildWorkbook(list(sheets[["PROBE_DATA"]], sheets[["LAB_DATA"]], sheets[["MANUAL_FIELD"]], ea_wq_data, lod_tab_data, exclusions_data, outliers_data, summary_tab_data, full_data, wide_output, desc))

### Apply sheet names
names(wb) <- c("PROBE_DATA", "LAB_DATA", "MANUAL_FIELD", "EA_DATA", "OUTWITH_LOD", "EXCLUSIONS", "OUTLIERS", "SUMMARY", "FULL_DATA", "WIDE_FORMAT", "DESCRIPTION")
