...
param_names <- names(merge)[16:ncol(merge)]
...
summary_1 <- full_data %>%
  ...
  dplyr::summarise(mn = mean(result, na.rm = TRUE),
                   ...
                   percentile = case_when(any(parameter == "temp")    ~ 0.98,
                                          ...
                                          .default = NA_integer_),
                   percentile_value = case_when(any(parameter == "temp")    ~ quantile(result, 0.98, na.rm = TRUE),
...
wb <- openxlsx::buildWorkbook(list(sheets[["PROBE_DATA"]], sheets[["LAB_DATA"]], sheets[["MANUAL_FIELD"]], lod_tab_data, exclusions_data, outliers_data, summary_tab_data, full_data))
names(wb) <- c("PROBE_DATA", "LAB_DATA", "MANUAL_FIELD", "OUTWITH_LOD", "EXCLUSIONS", "OUTLIERS", "SUMMARY", "FULL_DATA")
openxlsx::conditionalFormatting(wb, "FULL_DATA", cols = 17, rows = 2:nrow(full_data)+1,
                                rule = "$AF1<>''",
