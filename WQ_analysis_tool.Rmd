...
# If not importing APEM data (ie, just usign EA data), replace the path with NULL (no quotes)
TEMPLATE_PATH <- "APEM_input_template.xlsx"
...
SITES <- c("34307", "34308", "34309", "34310", "34311")
...
## Start date before 2000 (WQA database cut off for EA data)
...
if(is.null(SITES) == FALSE){
  wq_data <- hetoolkit::import_wq(sites = SITES,
...
if("PROBE_DATA" %in% names(sheets) == TRUE){
  ...
  probe_data <- extract_date_time(probe_data, "date_time")
