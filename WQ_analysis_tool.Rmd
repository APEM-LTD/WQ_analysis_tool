  ...
  for (p in unique(full_data$parameter)) {
    ...
    if (unique(chart_data$std_order == "asc")){
      chart<-ggplot(data = chart_data) +
        geom_rect(data = subset(chart_data, main_trib == "tributary")[1,], aes(fill = main_trib, xmin = x_min_val, xmax = x_max_val, ymin = y_min_val, ymax = y_max_val), fill = "#D9D9D9", alpha = 0.5) +
