...
# Specify style for time series plots
# line = WFD levels are delimted by coloured, dashed lines. Use this option if tributary rivers need to be highlighted
# block = WFD levels are delimted by coloured blocks. This option means tributary rivers cannot be highlighted.
CHART_TYPE <- "line"
  ...
  for (p in unique(full_data$parameter)) {
    ...
    chart_data <- full_data %>%
      ...
      dplyr::mutate(x_min_val = as.Date(paste0(as.character(lubridate::year(min(date))), "-01-01")),
                    ...
                    y_min_val = ifelse(y_min_val < 0, 0, y_min_val),
                    y_max_val = max(result, HIGH +10, POOR +10, na.rm = TRUE))
    
    y_min_val <- min(chart_data$y_min_val)
    y_max_val <- max(chart_data$y_max_val)
...
wide_summary <- summary_tab_data %>%
  tidyr::pivot_longer(cols = c(Mean, SD, Min, Max, Percentile, "Percentile value"),
                      names_to = "metric",
                      values_to = "value") %>%
  tidyr::pivot_wider(names_from = Parameter, values_from = value) %>%
  dplyr::arrange(Source, Location)
