---
title: "Water Quality Data Analysis System - WQDAS"
author: "APEM Ltd"
...
#   * Which data sheets within the APEM data template to import.
#   * A start date for the data (if required).
#   * An end date for the data (if required).
#   * Which WQ parameters to analyse (see the list in the PARAMETERS tab of the template for allowed params)
#   * Site IDs, both APEM and EA (recommended maximum is six sites per run). The order in which sites are specified will be reflected in the plots/tables.
#   * A multiplier to be applied to any data outwith the limits of detection
#   * A path to where the outputs should be saved (defaults to the Outpot folder within this project)
#   * Some settings to style the parameter charts
#    - Whether to show WFD standards as lines on the chart or as coloured
#    - Whether to format the shapes of the points on the charts to identify the source of the data 
#    - On the nitrate plots, whether to show the old reference standard at 11.3mg/L
#    - Whether to zoom the plots to the data (by default, all WFD standards are shown irrespective of the data range)
# * Save a copy of the script with your parameters specified.  
# * To run the script, use the Knit button above. This will render the report as an HTML document, saved to the top level project folder (see the files tab to the right). By default, the report will be called WQDAS_vX.html (the report name matches the name of this script file). 
######################################################################################################################
...
# Adjust min/max values to allow charts to zoom to data or to include the WFD standards as well
# The default ("standards") uses all data including WFD standards. Use "data" to zoom charts to the data. This is useful if, for example, a dataset is clustered at the bottom of a chart) 
CHART_ZOOM <- "standards"
...
# Convert chart type settings   to lower case
CHART_TYPE <- stringr::str_to_lower(CHART_TYPE)
CHART_ZOOM <- stringr::str_to_lower(CHART_ZOOM)
...
## Chart type is valid
if (CHART_ZOOM %in% c("standards", "data") == FALSE) {
  stop("The chart zoom setting must be either 'standards' or 'data'.")
}
...
metadata <- readxl::read_excel(paste0("Data/", TEMPLATE_PATH),
                             sheet = "METADATA") %>%    # Loads METADATA tab with details of the sites)
...
if(is.null(EA_SITES) == FALSE){
  ...
  ## Pull in blank file for matching on.
  ea_wq_data <- hetoolkit::import_wq(sites = "")
}
...
full_data <- full_data %>%
  dplyr::mutate(exclude = ifelse(result < 0 | result > 130, TRUE, FALSE),
                exclude = ifelse(parameter == "pH" & (result < 0 | result > 14), TRUE, exclude),
                exclude = ifelse(parameter == "temp" & (result < -10 | result > 100), TRUE, exclude))   # Add others as required
...
for (p in unique(full_data$parameter)) {
  ...
  if (CHART_ZOOM == "standards") {
    chart_data <- chart_data %>%
      dplyr::mutate(y_min = min(result, HIGH - 10, POOR, -10, na.rm = TRUE),
                    y_min = ifelse(y_min < 0, 0, y_min),
                    y_max = max(result, HIGH * 1.1, POOR * 1.1, na.rm = TRUE))
  } else if (CHART_ZOOM == "data") {
    chart_data <- chart_data %>%
      dplyr::mutate(y_min = min(result -10, na.rm = TRUE),
                    y_min = ifelse(y_min < 0, 0, y_min),
                    y_max = max(result * 1.1, na.rm = TRUE))
  }
...
## Create final summary table with status highlighted
...
## Pull required data from wide data
...
## Retain only required data for each parameter
...
## Rejoin all data (count of samples, second pH percentile data)
summary_report <- summary_report %>%
...
summary_report <- summary_report %>%
  ...
  dplyr::select(Location, period, n_samples, "Temperature (°C)", "DO (%)", "pH", "pH_95", "Ammoniacal Nitrogen as N (mg/L)", "Orthophosphate as P (mg/L)", "Biochemical Oxygen Demand (mg/L)")
...
## Extract standards 
stds <- full_data %>%
  dplyr::select(location_name, parameter, HIGH:POOR) %>%
  distinct() %>%
  dplyr::rename("Location" = "location_name") %>%
  dplyr::mutate(Parameter = case_when(parameter == "temp" ~ "Temperature (°C)",
                                      parameter == "RDO_sat" ~ "DO (%)",
                                      parameter == "BOD" ~ "Biochemical Oxygen Demand (mg/L)",
                                      parameter == "orthoP" ~ "Orthophosphate as P (mg/L)",
                                      parameter == "amm_N" ~ "Ammoniacal Nitrogen as N (mg/L)",
                                      .default = NA_character_))

## Calculate overall status
summ_rep <- summary_tab_data %>%
  dplyr::left_join(stds) %>%
  dplyr::mutate(STATUS = case_when(parameter == "temp" ~ ifelse(`Percentile value` < HIGH, "HIGH",
                                                         ifelse(`Percentile value` < GOOD, "GOOD",
                                                         ifelse(`Percentile value` < MODERATE, "MODERATE",
                                                         ifelse(`Percentile value` < POOR, "POOR", "BAD")))),
                                   parameter == "RDO_sat" ~ ifelse(`Percentile value` > HIGH, "HIGH",
                                                         ifelse(`Percentile value` > GOOD, "GOOD",
                                                         ifelse(`Percentile value` > MODERATE, "MODERATE",
                                                         ifelse(`Percentile value` > POOR, "POOR", "BAD")))),
                                   parameter == "BOD" ~ ifelse(`Percentile value` < HIGH, "HIGH",
                                                         ifelse(`Percentile value` < GOOD, "GOOD",
                                                         ifelse(`Percentile value` < MODERATE, "MODERATE",
                                                         ifelse(`Percentile value` < POOR, "POOR", "BAD")))),
                                   parameter == "orthoP" ~ ifelse(Mean < HIGH, "HIGH",
                                                         ifelse(Mean < GOOD, "GOOD",
                                                         ifelse(Mean < MODERATE, "MODERATE",
                                                         ifelse(Mean < POOR, "POOR", "BAD")))),
                                   parameter == "amm_N" ~ ifelse(`Percentile value` < HIGH, "HIGH",
                                                         ifelse(`Percentile value` < GOOD, "GOOD",
                                                         ifelse(`Percentile value` < MODERATE, "MODERATE",
                                                         ifelse(`Percentile value` < POOR, "POOR", "BAD")))),
                                   Parameter == "pH" ~ ifelse(`Percentile value` > 6 & `Percentile value` < 9,
                                                              "HIGH", "POOR")),
                parameter = ifelse(is.na(parameter), paste0(Parameter, "_", as.character(Percentile)), parameter))

## Keep only final status of each parameter
summ_rep <- summ_rep %>%
  dplyr::filter(!is.na(STATUS)) %>%
  dplyr::select(Location, parameter, STATUS) %>%
  tidyr::pivot_wider(names_from = parameter,
                     values_from = STATUS) %>%
  dplyr::rename(`Location name` = Location) %>%
  dplyr::select(c(`Location name`, temp, RDO_sat, pH_0.05, pH_0.95, amm_N, orthoP, BOD))

## Confirm rounding
summary_report <- summary_report %>%
  dplyr::left_join(summ_rep) %>%
  dplyr::mutate(across(all_of(col_names[4:10]), ~round(., 2)))

## Set up data table for report
  table <- DT::datatable(summary_report, 
                       ...
                       options = list(
                         ...
                         buttons = c('copy', 'csv', 'excel'),
                         columnDefs = list(
                           list(visible = FALSE, targets = 11:17    # Hide status columns 11 to 17
                           )
                         )
                       )
)
...
for (i in 4:10) {
  table <- table %>%
    DT::formatStyle(columns = i,
                  valueColumns = i + 7,
                  target = "cell",
                  # fontFamily = "Ariel, sans-serif",
                  # border = '0.5px solid #000000',
                  backgroundColor = styleEqual(c("HIGH", "GOOD", "MODERATE", "POOR", "BAD", ""),
                                               c("#B7DEE8", "#C4D79B", "#FFFF66", "#FFCC66", "#FF9999", "White")))
...
## Formal of overall summary
summary_report <- summary_report %>%
  dplyr::select(c("Location name", "Data period", "No. samples", "Temp (C, 98%ile)", "temp", "DO(%, 10%ile)", "RDO_sat", "pH(5%ile)", "pH_0.05", "pH(95%ile)", "pH_0.95", "Ammonia(mg/l, 90%ile)", "amm_N", "Orthophosphate(mg/l, mean)", "orthoP", "BOD(mg/l, 90%ile)", "BOD")) %>%
  dplyr::rename("Temp. status" = "temp",
                "DO status" = "RDO_sat",
                "pH (5%ile) status" = "pH_0.05",
                "pH (95%ile) status" = "pH_0.95",
                "Amm. N status" = "amm_N",
                "Ortho. P. status" = "orthoP",
                "BOD status" = "BOD")

wb <- openxlsx::buildWorkbook(list(summary_report, sheets[["PROBE_DATA"]], sheets[["LAB_DATA"]], sheets[["MANUAL_FIELD"]], ea_wq_data, lod_tab_data, exclusions_data, outliers_data, summary_tab_data, full_data, wide_output, wide_summary, standards, desc))

### Apply sheet names
names(wb) <- c("Results summary", "PROBE_DATA", "LAB_DATA", "MANUAL_FIELD", "EA_DATA", "OUTWITH_LOD", "EXCLUSIONS", "OUTLIERS", "SUMMARY", "FULL_DATA", "WIDE_FORMAT", "WIDE_FORMAT_SUMMARY", "Standards", "DESCRIPTION")
...
# openxlsx::conditionalFormatting(wb, "Results summary", cols = 4, rows = 0:nrow(summary_report),
#                                 rule = paste0("=$D2"))
